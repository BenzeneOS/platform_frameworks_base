...
class GosPackageStatePermission {
    ...
    static final int FIELD_SPOOFED_BATTERY_LEVEL = 3;

    @IntDef(prefix = "FIELD_", value = {
            FIELD_STORAGE_SCOPES, FIELD_CONTACT_SCOPES, FIELD_PACKAGE_FLAGS, FIELD_SPOOFED_BATTERY_LEVEL
    })
    ...
    GosPackageState filterRead(GosPackageState ps) {
        ...
        var res = new GosPackageState(ps.flagStorage1 & readFlagStorage1
                ...
                , canReadField(FIELD_SPOOFED_BATTERY_LEVEL) ? ps.spoofedBatteryLevel : default_.spoofedBatteryLevel
        );
    ...
    GosPackageState filterWrite(GosPackageState current, GosPackageState update) {
        ...
        var res = new GosPackageState(
                ...
                , canWriteField(FIELD_SPOOFED_BATTERY_LEVEL) ? update.spoofedBatteryLevel : current.spoofedBatteryLevel
        );
        var default_ = GosPackageState.DEFAULT;
        if (default_.equals(res)) {
            return default_;
