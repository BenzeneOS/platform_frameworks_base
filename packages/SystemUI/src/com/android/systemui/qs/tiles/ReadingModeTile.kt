...
class ReadingModeTile @Inject constructor(
    ...
    private var mPreviousDaltonizerMode = -1
    ...
    private fun setReadingModeEnabled(enabled: Boolean) {
        if (enabled) {
            // Save current daltonizer mode if it was already enabled with a different mode
            val wasEnabled = Settings.Secure.getInt(
                mContext.contentResolver,
                Settings.Secure.ACCESSIBILITY_DISPLAY_DALTONIZER_ENABLED, 0
            ) == 1
            if (wasEnabled) {
                ...
                if (currentMode != DALTONIZER_GRAYSCALE) {
                    mPreviousDaltonizerMode = currentMode
                }
            }
            ...
            // Restore previous daltonizer mode if we saved one
            if (mPreviousDaltonizerMode != -1 && mPreviousDaltonizerMode != DALTONIZER_GRAYSCALE) {
                Settings.Secure.putInt(
                    mContext.contentResolver,
                    Settings.Secure.ACCESSIBILITY_DISPLAY_DALTONIZER, mPreviousDaltonizerMode
                )
                mPreviousDaltonizerMode = -1
            } else {
                // Disable daltonizer
                Settings.Secure.putInt(
                    mContext.contentResolver,
